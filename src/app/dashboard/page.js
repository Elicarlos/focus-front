...
export default function PragmaDashboard() {
  ...
  const [microtasksOpen, setMicrotasksOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    // Carregar microtarefas
    const savedMicrotasks = localStorage.getItem("pragma_microtasks");
    if (savedMicrotasks) {
      try { setMicrotasks(JSON.parse(savedMicrotasks)); } catch (e) {}
    }
